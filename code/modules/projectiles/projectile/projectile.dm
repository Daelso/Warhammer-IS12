...
/obj/item/projectile
	...
	var/step_delay = 0.1 //1	// the delay between iterations if not a hitscan projectile
...
/obj/item/projectile/proc/launch(atom/target, var/target_zone, var/x_offset=0, var/y_offset=0, var/angle_offset=0)
	...
	if(targloc == curloc) //Shooting something in the same turf
		...
		qdel(src)
		return 0
	/*//Uncomment this if you want people to be able to shoot up again.
	if(targloc.z > curloc.z)
		var/turf/above = GetAbove(curloc)
		if(above.CanZPass(curloc, UP))
			curloc = above
	*/
